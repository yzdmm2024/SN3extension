//
//  Tweak.xm — 超级截图 v5.7 入口（SpringBoard + 聊天 App 双注入）
//
//  ────────────────────────────────────────────────────────────────────────
//  触发链：
...
//    + com.tencent.mqq / com.tencent.xin（v5.7 自动滚动长截图：驱动 UIScrollView 上报精确偏移）。
...
__attribute__((constructor)) static void xz_ctor() {
    @autoreleasepool {
        ...
        if ([procName isEqualToString:@"SpringBoard"]) {
            NSLog(@"[SN3] 超级截图 v5.7 loaded in SpringBoard");
            dispatch_async(dispatch_get_main_queue(), ^{
                @try {
